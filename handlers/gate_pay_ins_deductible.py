"""Shadow handler for WF | Gate | PayInsDeductible (Sales).

Spec source: workflow/01-gates/ev-to-dt/gate-pay-ins-deductible.md
- Trigger (live): Opportunity Changed, scoped to the Sales pipeline (Allow Re-Entry=Yes).
- IF: ev_ins_deductible_proof is not empty
- DO: if dt_ins_deductible_received is empty -> set it = Today (first receipt only)
- Idempotent: dt_ins_deductible_received is write-once.

Feeds the S45->S46 funding move and production-readiness (Insurance/Hybrid).

ACTIVE (SUPPORTS_WRITE=True; cut over 2026-06-30 after the output key was verified live via
/debug/field-keys). The matching GHL gate (WF | Gate | PayInsDeductible) stays Published until
you Draft it — harmless idempotent overlap, since both write the same write-once date. Per
docs/sales-gate-migration-plan.md.

Note: the GHL gate also flips Has|/Milestone|/Missing| Deductible tags — tag parity is phase 2
(our movers/readiness key on dt_*, not tags), so this handler stamps the date only.
"""
...
HANDLER_ID = "gate-pay-ins-deductible"
...
INPUT_FIELD = "ev_ins_deductible_proof"
OUTPUT_FIELD = "dt_ins_deductible_received"
...
async def execute(opp_data: dict[str, Any], decision: dict[str, Any]) -> dict[str, Any]:
    """Stamp dt_ins_deductible_received via the GHL writer (inert while shadow)."""
