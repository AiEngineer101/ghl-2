"""Unit tests for the WorkCompleted (TF→DT) shadow gate."""
import sys
from pathlib import Path

sys.path.insert(0, str(Path(__file__).resolve().parents[1]))

from handlers.gate_work_completed import INPUT_FIELD, OUTPUT_FIELD, evaluate  # noqa: E402
...
def test_no_op_when_tf_empty():
    assert evaluate(_opp(tf_value=""))["decision"] == "no_op"


def test_no_op_when_tf_missing():
    assert evaluate(_opp())["decision"] == "no_op"


def test_no_op_when_tf_no():
    assert evaluate(_opp(tf_value="No"))["decision"] == "no_op"
...
def test_accepts_list_form_yes():
    assert evaluate(_opp(tf_value=["Yes"]))["decision"] == "would_stamp"
