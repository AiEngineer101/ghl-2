"""Unit tests for the Sales pay-retail-deposit EV->DT gate."""
...
from handlers.gate_pay_retail_deposit import (  # noqa: E402
    INPUT_FIELD,
    OUTPUT_FIELD,
    PIPELINE_ID_SALES,
    evaluate,
...
def test_no_op_when_not_sales():
    r = evaluate(_opp({INPUT_FIELD: "http://deposit.pdf"}, pipeline_id="88V9uYY6visCrtI9V0NR"))
...
def test_would_stamp_when_proof_present_and_dt_empty():
    r = evaluate(_opp({INPUT_FIELD: "http://deposit/proof.pdf"}))
...
def test_skip_idempotent_when_dt_already_set():
    r = evaluate(_opp({INPUT_FIELD: "http://deposit/proof.pdf", OUTPUT_FIELD: "2026-06-20"}))
